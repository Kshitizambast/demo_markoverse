                    ...
                    <tbody>
                        @if(count($orders) > 0)
                            @foreach($orders as $order)
                                ...
                                @if($order->fullfilled == 1 and $order->my_shop_id == auth()->user()->id)
                                <tr id="orderrow{{$order->id}}">
                                    ...
                                    <td>
                                        ...
                                        @else
                                        <h5><span class="badge badge-danger-lighten"><i class="mdi mdi-coin"></i>cancelled</span></h5>
                                        @endif
                                    </td>
                                    <td>
                                        {{$order->paid}}
                                    ...
                                    <td>
                                       <button class="btn btn-sm  btn-danger" id="cancelorder{{$order->id}}">Cancel</button>
                                    </td>
                                    <script type="text/javascript">
                                        $(document).ready(() => {
                                         "use strict";
                                          ...
                                          $('#cancelorder{{$order->id}}').click(()=>{
                                            $('#orderrow{{$order->id}}').remove();
                                            axios.post('{{route('delete.byshop')}}',  {
                                               order_id : {{$order->id}},
                                               total    : {{$order->total}} 
                                            })
                                            .then((res)=>{
